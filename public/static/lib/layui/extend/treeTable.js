layui.define(['jquery'], function(exports) {
	...
	tree.prototype.template = function(e) {
		...
		o.each(t.data(e, e.data), function(idx, item) {
			var tr = '',
				is_checked = false,
				hide_class = (item[e.parent_key] == e.top_value || item[e.parent_key] == t.cache(e, item[e.parent_key]) || e.is_open) ? '' : e.hide_class;
			// 设置每行数据层级
			...
			o.each(e.cols, function(index, obj) {
				...
				// 第一次遍历头部的时候拼接表格头部
				idx == 0 && e.is_head && (thead += '<th ' + style + '>' + obj.title + '</th>');
				// 指定列加入开启、关闭小图标
				var icon = (obj.key == e.icon_key && !item.is_end) ? '<i class="' + (t.cache(e, item[e.primary_key]) || e.is_open ? e.icon.open : e.icon.close) + '"></i>' : '<span></span>';
				// 指定列小图标按照层级向后位移
		...
		o(e.elem).html(table).off('click', click_btn).on('click', click_btn, function() {
			...
			var is_checked = o(this).toggleClass('layui-form-checked').hasClass('layui-form-checked'),
				...
				id = tr.data('id'),
				pid = tr.data('pid');
			t.childs_checkbox(e, 0, id, is_checked);
	...
	// 子级反选
	tree.prototype.childs_checkbox = function(e, columnIdx, id, is_checked) {
		var t = this;
		o(e.elem).find('[data-pid=' + id + ']').each(function() {
			var checkbox = o(this).find('td').eq(columnIdx).find('.layui-form-checkbox');
			is_checked ? checkbox.addClass('layui-form-checked') : checkbox.removeClass('layui-form-checked');
			t.childs_checkbox(e, columnIdx, o(this).data('id'), is_checked);
		})
	};
	// 点击收起循环隐藏子级元素
	...
	// 获取已选值集合
	tree.prototype.checked = function(e, index, selIndex, isText) {
		var checked = [];
		if(index == undefined){
			index = 0;
		}
		...
		o(e.elem).find('tr').each(function(){
			if(o(this).find('td').eq(index).find('.layui-form-checked').length){
				var content = (index == selIndex && !isText ? o(this).data('id') : o.trim(o(this).find('td').eq(selIndex).text()));
	...
	tree.prototype.parentValues = function(e, id, index, isTitle, result){
		var ot = o(e.elem).find('[data-id='+id+']');
		if(ot.length && id){
			var pid = ot.data('pid');
			var val = ot.find('td').eq(index).text();
			isTitle ? result.unshift(val) : result.unshift(id);
			this.parentValues(e, pid, index, isTitle, result);
		}
		return result;
	}
	// 存储折叠状态
